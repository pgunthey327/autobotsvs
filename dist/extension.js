...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('extension.opencodeGenerator', async () => {
        panel = vscode.window.createWebviewPanel('textboxUI', // The internal ID of the webview
        ...
        if (panel) {
            ...
            try {
                ...
                for (let i = 0; i < parts.length; i += 2) {
                    ...
                    parts[i] = parts[i].replace(/(src|href)="([^\"]+)"/g, (match, attr, url) => {
    ...
    const copyAndPasteValueCommand = vscode.commands.registerCommand('extension.copyAndPasteValue', async (valueToCopy) => {
        ...
        function insertTextAtPosition(editor, text) {
